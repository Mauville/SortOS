public class ListaLigada<T extends Comparable<T>>{
    ...
    /*
     * Izquierda debe ser el primer elemento
     * Derecha debe ser el último elemento
     * 
     * */
    /*
     * public void quickSort() {
    	quickSort(this.inicial, );
    }
    
    public void quickSort(Nodo<T> inical, int izquierda, int derecha) {
    	T pivote= encontrarNodoEnElndice((derecha-izquierda)/2).getElemento();
    	int i= izquierda;
    	int j= derecha;
    	while (i<=j) {
			while (encontrarNodoEnElndice(i).getElemento().compareTo(pivote)>0) {
				i++;
			}
			while (encontrarNodoEnElndice(j).getElemento().compareTo(pivote)>0) {
				--j;
			}
			if (i<=j) {
				if (i!=j) {
    	...
    	if(izquierda<j) {
			quickSort(inicial, izquierda, j);
		}
    	if (i<derecha) {
			quickSort(inical, i, derecha);
		}
